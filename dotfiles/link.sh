...
# zellij reads its layouts from layout_dir at runtime, so those are rendered
# too — a literal $DOTFILES in one would never be expanded.
render zellij/config.kdl "$HOME/.config/zellij/config.kdl"
for layout in "$DOTFILES"/zellij/layouts/*.kdl; do
    [ -e "$layout" ] || break
    render "zellij/layouts/$(basename "$layout")" \
           "$HOME/.config/zellij/layouts/$(basename "$layout")"
done
