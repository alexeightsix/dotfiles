 ...
 *
 *   /send      flush every held message in FIFO order (Ctrl+Enter)
...
export default function (pi: ExtensionAPI) {
	...
	const render = () => {
		...
		const pausedHint = pausedByLimit
			...
			: isTimePaused()
				...
				: "    /send to send it now    /abort to cancel";
		const rows = [
	...
	pi.on("input", async (event, ctx) => {
		...
		// Commands must run immediately, and forwards never reach a model.
	...
	const sendNow = async (_args: string, ctx: { ui: { notify: (m: string, t: "info" | "warning") => void } }) => {
		if (queue.length === 0) {
			ctx.ui.notify(releasing ? "The held message is already being released." : "Nothing held.", "info");
			return;
		}
		for (const held of queue) held.sendAt = 0;
		const count = queue.length;
		if (pausedByLimit || isTimePaused()) {
			ctx.ui.notify(
				pausedByLimit
					? `${count} held message${count === 1 ? " is" : "s are"} paused by /limit.`
					: `${count} held message${count === 1 ? " is" : "s are"} paused for ${pauseLeft()}. /pause off to resume.`,
				"warning",
			);
			return;
		}
		flush();
		ctx.ui.notify(`Flushing ${count} held message${count === 1 ? "" : "s"} in order.`, "info");
	};

	pi.registerCommand("send", {
		description: "Flush every held message now, skipping all remaining countdowns",
		handler: sendNow as never,
	});
	pi.registerShortcut("ctrl+enter", {
		description: "Release every held prompt immediately",
		handler: (ctx) => void sendNow("", ctx),
	});
