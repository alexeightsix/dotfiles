 ...
 * which models did the work, what each one cost, how the time was spent, which
 * tools were called, and how many Claude subagents were spawned.
 *
...
export default function (pi: ExtensionAPI) {
	...
	const claudeSlots = new Set<string>();
	...
	// external-claude-agent.ts announces each subagent it starts.
	pi.events.on("claude:spawn", (data) => {
		const slot = (data as { slot?: string })?.slot;
		if (slot) claudeSlots.add(slot);
	});
	...
	pi.registerCommand("stats", {
		...
		handler: async (_args, ctx) => {
			...
			lines.push(
				claudeSlots.size === 0
					? "Agents   no Claude subagents spawned"
					: `Agents   ${claudeSlots.size} Claude subagent${claudeSlots.size === 1 ? "" : "s"} spawned (${[...claudeSlots].join(", ")}) — /claude for their spend`,
			);
