 ...
 * Permission Modes
 *
 * Claude-Code-style permission gating for pi.
 *
...
export default function (pi: ExtensionAPI) {
	...
	const showStatus = (ctx: ExtensionContext) => {
		...
		// Colour carries the meaning: red nothing stops it, gold you will be
		// asked, green it cannot change anything. With colour doing that work the
		// word "perm" is redundant, so the mode stands alone and saves a column.
		const color = mode === "all" ? "error" : mode === "ask" ? "warning" : "success";
		const label = mode === "read-only" ? "ro" : mode;
		const scope = grants.size > 0 ? theme.fg("dim", `+${grantScope()}`) : "";
		ctx.ui.setStatus("permission-mode", theme.fg(color, label) + scope);
		// claude-provider.ts spawns a stricter Claude when the mode is read-only.
		pi.events.emit("perm:mode", { mode });
	};
