...
function handoffInstructions(): string {
	for (const candidate of [
		path.join(process.env.HOME ?? "", ".agents/skills/handoff/SKILL.md"),
		path.join(agentDir(), "skills/handoff/SKILL.md"),
	]) {
		try {
			return fs.readFileSync(candidate, "utf-8").replace(/^---[\s\S]*?---\s*/, "").trim();
		} catch {
			// Try the next globally discovered skill location.
		}
	}
