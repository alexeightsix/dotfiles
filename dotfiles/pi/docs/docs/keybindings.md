...
| `Ctrl+Enter` | Release held prompts immediately (`/send`) |
...
| `Enter` | Queue a steering message while working |
...
The `pi-vim` package provides modal editing in the prompt. It is a `settings.json` package, not a keybinding override, so it composes with everything above.
