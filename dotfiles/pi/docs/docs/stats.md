...
By model
  openai-codex/gpt-5.6-sol    21 turns  ↑820.1k  ↓31.4k  $2.104  50%
  claude-code/claude           8 turns  ↑312.0k  ↓12.8k  $1.702  41%
  opencode-go/kimi-k3          5 turns  ↑71.4k   ↓4.0k   $0.375   9%
...
Agents   3 Claude subagents spawned (claude#1, claude#2, claude2#1) — /claude for their spend
```
...
**Spend** — total input (including cache reads and writes) and output tokens, and dollars. Costs come from each provider's reported usage; Claude Code turns report real subscription cost rather than a price-table estimate.
...
**By model** — the point of the command. Turns, tokens, cost, and share of total spend per model, sorted by cost. This is what tells you whether an expensive model is earning its place, and it is the feedback loop for tuning [`/drive`](./drive.md)'s routing table.

**Tools** — call counts by tool, from `tool_execution_start`.

**Agents** — Claude subagents spawned this session and the slots they used. `/claude` breaks down their individual token and dollar spend, which is tracked separately because subagents run outside Pi's own accounting — see [Claude subagents](./claude-subagents.md).

## Scope

Figures describe the **current branch** of the conversation. After a `/fork` or a jump in `/tree`, the token and model tallies reflect the branch you are on; the timing, tool, and agent counters are per-process and cover the whole session regardless of branch.
