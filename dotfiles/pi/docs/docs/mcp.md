...
`mcp.json` is the global override, linked to `~/.pi/agent/mcp.json`. It matches the servers configured for Claude Code and opencode, so the same capabilities are available whichever editor is open.
...
`hostConfigDiscovery` is `off`, so Pi uses only `mcp.json` and does not silently adopt Claude Code's or Codex's server lists. Add a server by editing `mcp.json` in this repository — it is the tracked copy.
