...
A question you asked for a second ago must not toast at you. Because the gate is "is this pane visible", that falls out for free: `/drive`'s picker, the forwarding session list and `/kill`'s confirmation are all triggered by you, at the terminal, so the check suppresses them with no special-casing.
