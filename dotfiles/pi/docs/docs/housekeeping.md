...
**Pi does not garbage-collect sessions.** Transcripts, exported HTML, shell logs, and improver conversations accumulate in the agent directory until something removes them. `/gc` is that something.
...
| `improve` | Yes — [improver](./improve.md) conversations and logs |
