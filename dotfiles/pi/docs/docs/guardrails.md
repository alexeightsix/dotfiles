...
2. **Switch model and continue** — choose another scoped model, give it Matt Pocock's redacted `handoff` workflow and the retained prompt, then continue there. The same token allowance restarts from zero for the new model; earlier branch spend remains visible in `/stats` and `/costs`.
