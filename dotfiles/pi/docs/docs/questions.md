...
Sending the next ordinary prompt clears the widget because that prompt is treated as the answer. Slash commands and forwarded messages do not clear it, so inspecting `/dash` or forwarding work elsewhere does not accidentally discard the reminder. `/questions` shows the pending questions in a notification, and `/questions clear` is the manual escape hatch.
