 ...
 * lets a write through without prompting, a limit parser that reads "$5" as 5
 * tokens, a prompt filter that mangles what a subagent receives. Everything
 * here is value-in, value-out — no pi process, no network.
 */
...
describe("forward wrapper", () => {
	const envelope = { from: "api", fromCwd: "/tmp/proj", at: "2026-08-10 04:30", text: "body" };

	it("is blank by default, so the target gets exactly what was sent", () => {
		assert.equal(applyWrapper({ prefix: "", suffix: "" }, envelope), "body");
	});

	it("fills placeholders", () => {
		assert.equal(
			applyWrapper({ prefix: "[{from} @ {at}] ", suffix: " ({cwd})" }, envelope),
			"[api @ 2026-08-10 04:30] body (/tmp/proj)",
		);
	});

	it("leaves unknown placeholders alone rather than blanking them", () => {
		assert.equal(applyWrapper({ prefix: "{nope} ", suffix: "" }, envelope), "{nope} body");
	});
});

describe("subagent prompt filtering", () => {
	// Shaped like pi's real prompt — see core/system-prompt.js.
	const piPrompt = [
		"You are an expert coding assistant operating inside pi, a coding agent harness.",
		"",
		"Available tools:",
		"- read: Read a file",
		"- todo_write: Record a plan",
		"",
	...
	const filtered = stripPiOnlySections(piPrompt);

	it("removes the parent's tool list", () => {
		// A subagent cannot call these; listing them invites confident calls to
		// tools that do not exist in its process.
		assert.ok(!filtered.includes("Available tools:"));
		assert.ok(!filtered.includes("todo_write"));
	});

	it("removes pointers into pi's own docs", () => {
		assert.ok(!filtered.includes("Pi documentation"));
	});

	it("keeps the guidelines and the project working order", () => {
		assert.ok(filtered.includes("Guidelines:"));
		assert.ok(filtered.includes("Be concise"));
		assert.ok(filtered.includes("Documentation is the source of truth."));
	});

	it("passes an unrecognised prompt through untouched", () => {
		// Upstream may reword its prompt; a no-op beats mangling it.
		const other = "Something else entirely.";
		assert.equal(stripPiOnlySections(other), other);
	});
});

describe("misc helpers", () => {
	it("addresses claude slots predictably", () => {
		assert.equal(slotId("claude", 1), "claude#1");
		assert.equal(slotId("claude2", 2), "claude2#2");
	});
	...
	it("keeps a visible gap between full-width dashboard tool names", () => {
		assert.equal(
			formatColumns(["forward_to_session", "todo_write"], 18, 4)[0],
			"forward_to_session  todo_write",
		);
	});
});
...
describe("pinned questions", () => {
	...
	it("clears only for ordinary user answers", () => {
		...
		assert.equal(isOrdinaryAnswer("<<<< forwarded", "interactive"), false);
