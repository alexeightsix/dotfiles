 ...
 * extension re-entry, non-TUI interception, and /send forcing the whole queue.
 */
...
async function harness(options: { mode?: string; hasUI?: boolean; acknowledge?: boolean } = {}): Promise<Harness> {
...
describe("send hold queue", () => {
	...
	it("never holds slash commands, forwards, or extension replays", async () => {
		const h = await harness();
		assert.equal((await h.input("/abort")).action, "continue");
		assert.equal((await h.input("<<<< [clipboard]")).action, "continue");
	...
	it("/send clears every countdown, not only the head", async () => {
		const h = await harness();
		await h.input("urgent");
		await settle(15);
		await h.input("also urgent");
		await h.command("send");
	...
	it("Ctrl+Enter is a keyboard alias for /send", async () => {
		const h = await harness();
		await h.input("urgent");
		await h.shortcut("ctrl+enter");
		assert.deepEqual(h.sent, ["urgent"]);
	});
	...
	it("/abort all is an escape hatch for a release that receives no acknowledgement", async () => {
		...
		assert.equal(h.widget(), undefined);
		await h.command("send");
		assert.match(h.notes.at(-1) ?? "", /Nothing held/);
	});
