 ...
 * directory, and derives a stable per-project port so several projects can
 * serve their docs at once.
 *
...
/** Stable port per project, so two projects do not fight over one. */
function portFor(docsDir: string): number {
	let hash = 0;
	for (let i = 0; i < docsDir.length; i++) hash = (hash * 31 + docsDir.charCodeAt(i)) >>> 0;
	return 3200 + (hash % 400);
}
